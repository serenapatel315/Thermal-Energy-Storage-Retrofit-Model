...
function TES_dispatch_rep(fname,threads,path)
    ...
    #coal 
    coal_fuel_cost = parameters_dict["coal_fuel_cost"] #0.047715308 1k/MWh 
    coal_VOM = parameters_dict["coal_VOM"] #0.09712 1k/MWh --> too high
    ...
    # @constraint(model, coal_dispatch_constraint, 500*8736*0.8 <= coal_dispatch)
    ...
    if TES_setting == 2 #force TES option
        @constraint(model,TES_binary_f, x_TES == 1) 
    end
    ...
    if BaselineEmissions > 0 
       ...
       println("EMISSIONS APPLIED",BaselineEmissions," ",emissions_percent)
    end
